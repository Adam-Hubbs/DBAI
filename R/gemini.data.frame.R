  ...
  if(repair == TRUE) {
    for (iter in 1:iterations) {
      for (h in 1:max_len) {
        ...
        source[[outputcol]][na_index] <- gemini(source = na_input, prompt = prompt[h], progress = progress, model = model[h], temperature = temperature[h], top_p = top_p[h], max_tokens = max_tokens[h], parentInfo = parentInfo)
      }
    }
  } else {
    source <- source |>
    ...
    for (iter in 1:iterations) {
      for (h in 1:max_len) {
        ...
        source <- source |>
          dplyr::mutate(!!outputcol := gemini(source = !!sym(input), prompt = prompt[h], progress = progress, model = model[h], temperature = temperature[h], top_p = top_p[h], max_tokens = max_tokens[h], parentInfo = parentInfo))
      }
    }
  }
