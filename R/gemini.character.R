  ...
  completion <- function(input, prompt) {
    ...
    if (httr::http_error(response)) {
      ...
      if(httr::status_code(response) == 404) {
        cli::cli_abort(c("Gemini API request failed [404]."), footer = {parsed$error$message}, call = call, class = "404")
      } else {
        cli::cli_abort(c("Gemini API request failed [{httr::status_code(response)}]", x = "{parsed$error$message}"), call = call)
      }
    }
